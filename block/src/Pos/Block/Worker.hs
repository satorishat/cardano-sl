...
import           Pos.Slotting (ActionTerminationPolicy (..), OnNewSlotParams (..),
                               currentTimeSlotting, getSlotStartEmpatically)
...
informerWorker =
    onNewSlotWorker def announceBlockOuts $ \slotId _ ->
...
blkCreatorWorker =
  ...
  where
    ...
    onsp =
        def {onspTerminationPolicy = NewSlotTerminationPolicy "block creator"}
